...
83 real command descriptors, 45 zero-filled filler descriptors, and `SCRPUT`
...
core-bank $1A40–$1B3F, code at assigned code-bank $8000, logical module 6 /
...
The built-in descriptor at $1A20 belongs to UMHZ, so use matching package builds.
...
Current source macro names such as `CMD_LOW`, `CMD_LOW_ALL`,
...
`build_support/build_readybasic_disk_modules.py`. `LDMOD(name$)` lives in the
...
| Prefix | Use |
| --- | --- |
| Demo packages | Examples have plain names and are loaded with LDMOD. |
...
Production and disk-demo names include `ECHO1`, `ADD16`, `UPPER`, `LOWER`,
`HIDDENRAM`, `SUMNUMARRAY`, `RANGENUMARRAY`, `TEMPSCRATCH`, `PAUSE`,
`FAIL`, `MEMAVL`, `ERRCODE`, `ERRLINE`, `BUFMAKE`, `BUFFILL`, `BUFDROP`,
`SCRCAP`, `SCRPUT`, `FADD`, `SLOT0`, `SLOT1`, `SLOT2`, `SPAN`, `OVL1`,
`OVL2`, `CPYRST`, `COPY`, and `LDMOD`. Disk-module samples add `DM1`,
`DM2S`, `DOV1`, and `DOV2` after `LDMOD("RBM.SAMPLE1")` or
`LDMOD("RBM.SAMPLE2")`. The larger `LDMOD("RBM.SAMPLE3")` proof adds
`S6AA` through `S8EB`, each returning a stateful integer sentinel. The command
name itself encodes submodule family, overlay, and entrypoint: `S6`/`S7`/`S8`
...
1. A parser signature such as `parse_sig_add16`. This runs in resident
   ...
   frame (`CF_*` fields).
2. A worker label such as `cmd_add16_low`. This is the overlay entry point
   copied from REU into C64 RAM. It reads `CF_*` fields and writes the result
   frame (`RF_*` fields).
3. A package entry such as `("ADD16", "cmd_add16_low", 2)` in `SAMPLE1_LOW`. The module builder emits the 32-byte
...
Production commands use the resident descriptor macros. Disk examples use
`SAMPLE1_LOW`, `SAMPLE2_GROUPS` and `rbm3_payload_commands()` in the package
builder. The binary 32-byte descriptor format is the same for both.

## ECHO1: Smallest Scalar Command

ECHO1 is a real disk worker in `sample_low.s`. Load sample1 first:
...
The package entry `("ECHO1", "cmd_echo1_low", 14)` selects the existing
no-argument integer-output parser. Statement form captures/clears P%; expression
form takes no output actual. The dispatcher copies the sample payload to slot 0,
executes this worker and commits RF_VAL. No resident constant-return shortcut
or built-in ECHO1 descriptor remains.

## ADD16: Numeric Inputs Plus Output

`ADD16` proves two numeric expressions followed by an integer output variable.
...
```asm
; SAMPLE1_LOW package entry: ("ADD16", "cmd_add16_low", 2)
```
...
| `SAMPLE1_LOW: ("ADD16", "cmd_add16_low", 2)` | Tells the registry this command uses the scalar-add parser and its entry offset in the shared disk sample payload. |
...
## HIDDENRAM: Under-ROM Worker Command

`HIDDENRAM` proves that a command can run under BASIC ROM RAM in the current
disk sample1 slot-0 payload at `$A800`. Its descriptor uses the same
module format as production commands. ReadyBASIC fetches the payload into `$A800`, maps
...
10 HIDDENRAM("AB",H%)
20 PRINT "HIDDEN";H%
30 PRINT "EXPR";HIDDENRAM("A")
```

```asm
; SAMPLE1_LOW package entry: ("HIDDENRAM", "cmd_hiddenram_hidden", 5)
```
...
| Field | Meaning |
| --- | --- |
| Sample package command ID | Assigned by SAMPLE1_LOW ordering in the package builder. |
| `SIG_HIDDENRAM` | Parser signature that stages a string input and integer output. |
| `cmd_hiddenram_hidden` | Historical label for the slot-0 under-ROM worker entry copied to `$A800`. |
| `cmd_hiddenram_hidden_end` | Worker boundary; the package copies the shared sample payload and uses its entry offset. |
| `"HIDDENRAM"` | Public command text; this is a disk-only proof command in sample1. |
...
## SUMNUMARRAY And RANGENUMARRAY: Array References

ReadyBASIC array parameters are explicit: pass the base element and a count.

`SUMNUMARRAY` invocation examples:

```basic
5 LDMOD("RBM.SAMPLE1",M%)
10 DIM A%(3)
20 A%(0)=1:A%(1)=2:A%(2)=3
30 SUMNUMARRAY(A%(0),3,S%)
40 PRINT "SUM";S%
50 PRINT "SUMEX";SUMNUMARRAY(A%(0),3)
```
...
`RANGENUMARRAY` is the output-array sibling. It stages integer bytes in the
...
`RANGENUMARRAY` invocation example:

```basic
5 LDMOD("RBM.SAMPLE1",M%)
10 DIM R%(4)
20 RANGENUMARRAY(7,4,R%(0))
...
```asm
; SAMPLE1_LOW package entry: ("SUMNUMARRAY", "cmd_sumnumarray_low", 6)
; SAMPLE1_LOW package entry: ("RANGENUMARRAY", "cmd_rangenumarray_low", 7)
```
...
| `RANGENUMARRAY` descriptor | Publishes the sibling command that stages multiple integer outputs through the result frame. |
...
5 LDMOD("RBM.SAMPLE1",M%)
10 MEMAVL()
20 P%=TEMPSCRATCH(512)
30 PRINT "PAGES";P%
40 TEMPSCRATCH(512,Q%)
50 PRINT "OUT";Q%
```
...
`FAIL` intentionally demonstrates error cleanup. The output variable is cleared
...
5. Add a disk package entry for examples, or a production descriptor for built-ins.
   Example workers must not be linked into the production packs.
...
| `__LOWPACK_SIZE__` | Measured size of the current slot-0 payload, measured in `obj/readybasic.map`. |
...
| `__SLOTPACK2_LOAD__`, `__OVL1PACK_LOAD__`, `__OVL2PACK_LOAD__` | Production graphics payload symbols. Disk proof payloads are assembled/generated separately. |
...
| `cmd_*_hidden` | Historical label suffix for the `HIDDENRAM` worker now handled through unified slot-0 under-ROM dispatch. |
...
`ABS(ADD16(1,6)-10)`, `ABS(ADDI(1,6)-10)`, and
