...
emit_type_step "load_demo_commands" $'PRINT LDMOD("RBM.SAMPLE1")\r' "$STEP_POST"
emit_type_step "demo_01_setup" $'MEMAVL()\rNEW\rA%=42\r10 B%=ADD16(42,8)\r20 PRINT "PROGRAM SUM";B%\r' "$STEP_POST"
...
emit_type_step "demo_04_assembler_setup" $'N%=ADD16(7,8)\rT$=UPPER("ready")\rL$=LOWER("LOUD")\rH%=HIDDENRAM("AB")\rPRINT CHR$(158);"EXPECT: 15, READY, LOWER, 131"\rPRINT CHR$(5)\r' "$READ_PAUSE"
...
emit_type_step "demo_06_parameter_groups" $'DIM A%(3):A%(0)=1:A%(1)=2:A%(2)=3\rS%=SUMNUMARRAY(A%(0),3)\rDIM R%(4):RANGENUMARRAY(7,4,R%(0))\rF=FADD(1.2,2.3)\rPRINT CHR$(158);"EXPECT: SUM 6, RANGE 7..10, FADD 3.5"\rPRINT CHR$(5)\rPRINT "SUM";S%\rPRINT "RANGE";R%(0);R%(3)\rPRINT "FADD";F\r' "$READ_PAUSE"
...
emit_type_step "demo_09_intro" $'PRINT CHR$(147);CHR$(158);"DEMO 10: CONTROLLED ERROR"\rPRINT "FAIL SHOWS READYBASIC ERROR CLEANUP":PRINT CHR$(5)\r' "$READ_PAUSE"
emit_type_step "demo_09_fail" $'PRINT CHR$(158);"EXPECT: RB ERROR 7 AND X% BECOMES ZERO"\rPRINT CHR$(5)\rX%=99\rFAIL(7,X%)\rPRINT "AFTER FAIL";X%\r' "$STEP_POST"
...
emit_type_step "demo_10_program" $'NEW\r10 FUNC ADDI(X%,Y%)\r20  RET X%+Y%\r30 ENDP\r40 FUNC GREET(N$)\r50  RET "HI "+N$\r60 ENDP\r70 PRINT "ABS";ABS(ADD16(1,6)-10)\r80 PRINT "LEFT ";LEFT$(UPPER("ready"),2)\r90 PRINT "NEST";ADD16(1,ADD16(2,3))\r100 PRINT "FABS";ABS(FADD(1.2,2.3)-3)\r110 PRINT "FLEFT ";LEFT$(GREET("READY")+"!",3)\r' "$STEP_POST"
