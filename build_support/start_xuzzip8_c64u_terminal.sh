...
speed="${XUZZIP8_SPEED_MHZ:-16}"
quiet_s="${XUZZIP8_QUIET_S:-600}"
stamp="$(date +%Y%m%d_%H%M%S)"
log="${XUZZIP8_START_LOG:-/tmp/xuzzip8_c64u_${stamp}.log}"
status_file="${XUZZIP8_START_STATUS:-/tmp/xuzzip8_c64u_${stamp}.status}"
runner_snapshot="/tmp/run_xuzzip8_c64u_${stamp}.sh"
...
command_text="cd '$readyos_root' && C64U_HOST='$host' XUZDEFLATE_ARCHIVE_MODE=zip8 XUZDEFLATE_SPEED_MHZ='$speed' XUZDEFLATE_QUIET_S='$quiet_s' /bin/bash '$runner_snapshot' '$log' '$status_file'"
osascript -e 'tell application "Terminal" to do script "'"${command_text//"/\\"}"'"' >/dev/null
echo "Started Terminal-owned physical xuzzip8 at ${speed} MHz"
