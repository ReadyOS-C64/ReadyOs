...
stream_port="${XUZCREATEPLAN_STREAM_PORT:-12246}"
log="${1:-/tmp/xuzcreateplan_c64u_${speed}mhz.log}"
status_file="${2:-/tmp/xuzcreateplan_c64u_${speed}mhz.status}"
connect_wait_s="${C64U_CONNECT_WAIT_S:-300}"
run_id="XUZCREATEPLAN-$(date +%Y%m%d-%H%M%S)-${speed}MHZ-$$"
remote_parent="USB1/READYOS_UZIP_TEST"
remote_root="$remote_parent/$run_id"
out_dir="${XUZCREATEPLAN_OUT_DIR:-$readyos_root/logs/xuzcreateplan/$run_id}"
fixtures="$out_dir/fixtures"
boot_plan="$out_dir/xuzcreateplan-boot-plan.yaml"
plan_run="$out_dir/xuzcreateplan-plan.yaml"

case "$speed" in 1|16|64) ;; *) echo "speed must be 1, 16, or 64 MHz" >&2; exit 64 ;; esac
case "$remote_root" in
  USB1/READYOS_UZIP_TEST/XUZCREATEPLAN-*) ;;
  *) echo "refusing non-owned xuzcreateplan root: $remote_root" >&2; exit 64 ;;
esac

mkdir -p "$out_dir" "$fixtures" "$readyos_root/obj/xuzcreateplan"
...
{
  date
  echo "Physical C64 Ultimate recursive create-plan diagnostic"
  echo "Host: $host"
  echo "CPU speed: ${speed} MHz"
  echo "Owned remote root: /$remote_root"
  echo "Storage mutation by C64: none"
  echo "Preserve root: yes"
  echo "VICE use: forbidden"
} > "$log"

cd "$readyos_root" || exit 1
run python3 build_support/build_xuzcreateplan_fixture.py "$fixtures" \
  --run-id "$run_id" --header obj/xuzcreateplan/xuzcreateplan_config.h || exit 1
run python3 build_support/verify_uci_protocol_contract.py || exit 1
run env UZIP_CREATEPLAN_DIAGNOSTIC=1 /bin/bash ./run.sh \
...
cp "$d81" "$out_dir/xuzcreateplan-diagnostic.d81"
cp bin/uzip.prg "$out_dir/uzip-diagnostic.prg"
shasum -a 256 "$out_dir/xuzcreateplan-diagnostic.d81" \
...
while IFS= read -r relative; do
  run curl --fail --silent --show-error --ftp-create-dirs --max-time 90 \
    -T "$fixtures/SOURCE/$relative" \
    "$ftp_base/${remote_root}/SOURCE/$relative" || exit 2
done < <(python3 - "$fixtures/manifest.json" <<'PY'
import json
import sys
...
run curl --fail --silent --show-error --max-time 60 \
  --quote "MKD /${remote_root}/SOURCE/TOP/EMPTY" \
  "$ftp_base/" || exit 2
...
if ! run env C64U_HOST="$host" C64U_SKIP_UPLOAD=1 C64U_SKIP_CONFIG=1 \
  ...
  "$out_dir/xuzcreateplan-diagnostic.d81" XUZCREATEPLAN-CLEAR.D81 \
...
run curl --fail --silent --show-error --ftp-create-dirs --max-time 300 \
  -T "$out_dir/xuzcreateplan-diagnostic.d81" \
  "$ftp_base/${remote_root}/XUZCREATEPLAN.D81" || exit 2
run curl --fail --silent --show-error --max-time 30 -X PUT \
...
run curl --fail --silent --show-error --max-time 60 -X PUT \
  "http://${host}/v1/drives/a:mount?image=%2F${encoded_remote_root}%2FXUZCREATEPLAN.D81&type=d81&mode=readwrite" \
...
sed -e "s#host: 10.0.0.79#host: ${host}#g" \
  -e "s#USB1/READYOS_UZIP_TEST/XUZCREATEPLAN-RUN#${remote_root}#g" \
  -e "s#stream_port: 12246#stream_port: ${stream_port}#g" \
  build_support/xuzcreateplan_boot_ultimate.generated.yaml > "$boot_plan"
...
case "$quiet_s" in
  ''|*[!0-9]*) echo "quiet observation seconds must be an integer" >> "$log"; exit 64 ;;
esac
if (( quiet_s < 10 || quiet_s > 1800 )); then
  echo "quiet observation seconds must be between 10 and 1800" >> "$log"
  exit 64
fi
sed -e "s#host: 10.0.0.79#host: ${host}#g" \
  -e "s#USB1/READYOS_UZIP_TEST/XUZCREATEPLAN-RUN#${remote_root}#g" \
  -e "s#stream_port: 12246#stream_port: ${stream_port}#g" \
  -e "s#post_delay_s: 180.0#post_delay_s: ${quiet_s}.0#g" \
  build_support/xuzcreateplan_ultimate.generated.yaml > "$plan_run"
...
run curl --fail --silent --show-error --max-time 30 \
  "http://${host}/v1/configs/U64%20Specific%20Settings" \
  -o "$out_dir/speed-pre-plan.json" || exit 2
python3 - "$out_dir/speed-pre-plan.json" "$speed" "$expected_turbo" >> "$log" 2>&1 <<'PY' || exit 2
import json
import sys
settings = json.load(open(sys.argv[1], encoding="utf-8"))["U64 Specific Settings"]
...
print(f"live CPU confirmed before create plan: turbo={actual_turbo} speed={actual_speed} MHz")
PY

cd "$harness" || exit 1
if ! ULTIMATE_ASSUME_MOUNTED=1 /usr/local/share/dotnet/dotnet run \
  --project src/ViceTasks.Binary/ViceTasks.Binary.csproj \
  -- run-ultimate-plan --plan "$plan_run" --no-tui >> "$log" 2>&1; then
  echo "physical Ultimate create-plan run reported failure; analyzing result" >> "$log"
fi

cd "$readyos_root" || exit 1
run python3 build_support/analyze_xuzcreateplan_run.py "$log" "$fixtures" \
  --json-output "$out_dir/result.json" || exit 4

run curl --fail --silent --show-error --max-time 60 --list-only \
  "$ftp_base/${remote_root}/" -o "$out_dir/root-after.txt" || exit 5
if tr '[:lower:]' '[:upper:]' < "$out_dir/root-after.txt" | \
   grep -Fqx "RESULT.ZIP"; then
  echo "read-only planner unexpectedly created RESULT.ZIP" >> "$log"; exit 5
fi
readback="$out_dir/source-readback"
mkdir -p "$readback"
while IFS= read -r relative; do
  local_file="$readback/$relative"
  mkdir -p "$(dirname "$local_file")"
  run curl --fail --silent --show-error --max-time 90 \
    "$ftp_base/${remote_root}/SOURCE/$relative" -o "$local_file" || exit 5
  cmp -s "$fixtures/SOURCE/$relative" "$local_file" || exit 5
done < <(python3 - "$fixtures/manifest.json" <<'PY'
import json
import sys
manifest = json.load(open(sys.argv[1], encoding="utf-8"))
...
echo "read-only source/output oracle passed" >> "$log"
echo "XUZCREATEPLAN PHYSICAL PASS ${speed}MHz /$remote_root" | \
