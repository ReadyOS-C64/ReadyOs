...
REQUIRED: dict[str, tuple[str, ...]] = {
    "README.md": (
        ...
        "ReadyOS now ships the same runtime in `12` public media variants",
        "| `precog-d81` | one `D81` image on drive `8` | recommended main ReadyOS SKU",
        "| `precog-ultimate` | one `D81` image on drive `8` | C64 Ultimate-first",
        "| `precog-dual-d71` | two boot-time `D71` images on drives `8` and `9`, plus an optional third `D71`",
    ...
    "src/apps/readme/readme_lite.md": (
        ...
        "`precog-ultimate`: the full D81 plus standalone `SETUP`",
        "`precog-dual-d71`: 16 core apps on two boot-time `1571` disks",
        "`precog-dual-d64`: reduced 8-app set",
        "`precog-solo-d64-readybasic`: ReadyOS, ReadyBASIC, all modules",
        "### ReadyBASIC",
        "### ReadyIRC",
        "### UCI Tester",
    ),
    ...
    "docs/ultimate_setup.md": (
        "focused ReadyOS TUI micromodules",
        "create new `rdyset.seq`",
        "rename Ultimate DOS name `apps.cfg.seq` to `rdyset.bak.seq`",
        "1, 16, and 64 MHz",
    ),
    "docs/ultimate_setup.html": (
        "focused ReadyOS TUI",
        "<code>rdyset.seq</code>",
        "<code>rdyset.bak.seq</code>",
        "passed on 2026-08-22 at 1, 16, and 64",
    ),
